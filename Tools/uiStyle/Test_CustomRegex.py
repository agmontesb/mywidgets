...
class TestExtcompile:
    ...
    def test_namedvars(self):
        ...
        assert (actual.tagPattern, actual.tags, actual.varList) == (
    ...
    def test_equivNotationII(self):
        ...
        scnd = CustomRegEx.compile('(?#<table id td{1.*=grp1 2{b.*=grp2 a{href=grp2a src=grp2b}} 3.*=grp3 4.*=grp4}>)',
                                   0)
        assert ExtCompObjEquality(first, scnd)
...
"""
    ...
    def test_general(self):
        ...
        answer = CustomRegEx.findall('(?#<son id=id href=url href1=_url_ grandson{href=__url__ *=label}>)',
                                     self.htmlStr)
