...
def createParser():
    ...
    parserdefaults = {'header': '{}', 'debug': False, 'referer': None, 'cookie_jar': None, 'post302': False,
                      'proxy': None, 'remote_header_name': False, 'proxy_auth': 'basic', 'req_method': '',
                      'create_dirs': False, 'location': False, 'include': False, 'form': '', 'cookie': None,
                      'user': None, 'post301': False, 'remote_name': False, 'data': '[]', 'auth_method': 'basic',
                      'proxy_user': None, 'url': None, 'data_binary': None, 'data_raw': None, 'user_agent': None,
                      'output': None, 'data_urlencode': None, 'post303': False}
    ...
    if opt_str in ['-d', '--data']:
        if value.startswith('@'):
            ...
            with open(fileName, 'r') as f:
                value = '&'.join(urllib.parse.urlencode(f.readlines()))
        ...
        if spos1 == spos2:
            ...
            name = value[:spos]
            if value[spos] == '=':
                value = urllib.parse.quote_plus(value[spos + 1:])
            else:
                fname = value[spos + 1:]
                with open(fname, 'r') as f:
                    value = urllib.parse.quote_plus(f.read())
...
def formProc(option, opt_str, value, parser):
    ...
    BOUNDARY = 6 * '-' + 'hostipaddr.uid.pid.timestamp.random' if not parser.values.form else parser.values.form.partition('\n')[
        0]
    items = {}
    ...
    while suffix:
        ...
        if 'filename' in items: joinBlk += '; filename="{filename}"'
        if 'type' in items: joinBlk += '\r\nContent-Type: {type}'
        joinBlk += '\r\n\r\n{value}\r\n'
...
class network:
    ...
    def parseCommand(self, curlCommand):
        ...
        if headers:
            ...
            strHeaders = ' '.join(['-H "%s: %s"' % (key, value[0]) for key, value in list(headers.items())])
    ...
    def processData(self, values, data):
        ...
        if cookie_jar and os.path.isfile(cookie_jar):
            cj = http.cookiejar.LWPCookieJar(cookie_jar)
            cj.extract_cookies(response, request)
            cj.save(cookie_jar)

        filename = ''
        if values.remote_name:
            filename = urllib.parse.urlparse(response.geturl()).path
    ...
    def getOpener(self, values):
        opener_handlers = [urllib.request.HTTPHandler(debuglevel=values.debug)]
        if hasattr(http.client, 'HTTPS'):
            opener_handlers.append(urllib.request.HTTPSHandler(debuglevel=values.debug))
        ...
        opener_handlers.append(
            netHTTPRedirectHandler(location=values.location, include=include, postSwitches=pSwitches))
        ...
        if values.user:
            ...
            if not password:
                try:
                    ...
                    password = eval(input("Enter password for " + user))
            passwordManager.add_password(None, values.url, user, password)
            opener_handlers.append(urllib.request.HTTPBasicAuthHandler(passwordManager))
            if values.auth_method == 'digest':
                opener_handlers.append(urllib.request.HTTPDigestAuthHandler(passwordManager))
            pass

        if values.proxy:
            proxyStr = values.proxy
            protocol, user, password, proxyhost = urllib.request._parse_proxy(proxyStr)
            protocol = protocol or 'http'
            proxy_handler = urllib.request.ProxyHandler({protocol: proxyhost})
            ...
            if user and not password:
                try:
                    ...
                    eval(input("Enter proxy password for " + user))
                passwordManager.add_password(None, proxyhost, user, password)
                opener_handlers.append(urllib.request.ProxyBasicAuthHandler(passwordManager))
                if values.proxy_auth == 'digest':
                    opener_handlers.append(urllib.request.ProxyDigestAuthHandler(passwordManager))
            pass
    ...
    def getRequest(self, values):
        ...
        headers = dict((key.encode('utf-8'), value.encode('utf-8')) for key, value in list(headers.items()))

        urlStr = values.url
        request = urllib.request.Request(urlStr, postdata, headers)
...
class LogNetFlow:
    ...
    def logRequest(self, request):
        ...
        for key, value in sorted(request_headers.items()):
            request_data += b'\n' + key + b': ' + value
        self.log += request_data.decode('utf-8')
        pass
...
class LogHandler(urllib.request.BaseHandler):
    '''
    ...
    def __init__(self, values):
        scheme = urllib.parse.urlsplit(values.url)[0]
    ...
    def requestProcessor(self, request):
        if self.include:
            ...
            if request.unredirected_hdrs:
                request_data += self.HEAD_SEP
                for key, value in list(request.unredirected_hdrs.items()): request_data += '\n' + key + ': ' + str(value)
    ...
    def errorProcessor(self, req, fp, code, msg, headers):
        ...
        if self.location:
            ...
            raise urllib.error.URLError('Redirection not allowed')
        pass


class netHTTPRedirectHandler(urllib.request.HTTPRedirectHandler):
    ...
    def http_error_302(self, req, fp, code, msg, hdrs):
        ...
        return urllib.request.HTTPRedirectHandler.http_error_301(self, req, fp, code, msg, hdrs)
